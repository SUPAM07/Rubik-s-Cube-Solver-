...
print(rf"{GREEN}         | |____   |  |___|  |  | |_____| |  | |_______                                  |_SU_|_PA_|_M_|            ")
...
def solve(state):
    ...
    for i in state:
        for j in state[i]:
            raw+=sign_conv[j]
    print("Solution:",Cube.solve(raw))
    return Cube.solve(raw)

def color_detect(h, s, v):
    """
    This is the calibrated function that correctly
    distinguishes between your specific cube's colors.
    """
    if s < 38:
        return 'white'
    if (h >= 0 and h <= 1) or (h >= 170 and h <= 179):
        return 'red'
    elif h >= 2 and h <= 15:
        return 'orange'
    elif h >= 18 and h <= 38:
...
def process(operation):
    ...
    for i in operation:
        ...
        texton_preview_stickers(solution_preview, stickers)
        cv2.putText(solution_preview, f"Move: {i}", (650, 50), font, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.imshow('Solution', solution_preview)
        if cv2.waitKey(2000) & 0xFF == 27: # Wait 500ms, allow Esc to close
            break
...
if __name__ == '__main__':
    ...
    while True:
        # Clear the preview window to black each frame
        preview[:] = (0, 0, 0)
        ...
        if k == 27:
            ...
            if len(set(check_state)) == 6:
                try:
                    solved_moves = solve(state)
                    if solved_moves:
                        operation = solved_moves.split(' ')
                        process(operation)
                except Exception as e:
                    print(f"Error solving cube: {e}")
                    print("Error in side detection. You may not have followed the sequence or some colors were not detected well. Please try again.")
            else:
                print("All sides are not scanned. Check the preview window to see which are left.")
                print(f"Sides left to scan: {6 - len(set(check_state))}")
